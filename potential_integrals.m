...
%     -1i * sqrt(volQ/(2*pi))*exp(1i*xi_midp'*x) .* dsinc(x/2*volQ); % old
    ...
    for k=1:n+1
        for l=1:n+1
            integral1(k,l)  = simpson_integral_vector( conj(e(k,:)).*e(l,:).*W1, x);
            integral2(k,l)  = simpson_integral_vector( conj(e(k,:)).*e(l,:).*W2, x);
            corrector1(k,l) = simpson_integral_vector( conj(cor(k,:)).*cor(l,:).*W1, x);
            corrector2(k,l) = simpson_integral_vector( conj(cor(k,:)).*cor(l,:).*W2, x);
        end
    end
end
